...
public class GunBenchTeachMenu extends TeachRecipeMenu<RecipeHolder<GunBenchRecipe>, GunBenchRecipe.Input>
{
	...
	@Override
	protected IMenuRecipeValidator<RecipeHolder<GunBenchRecipe>, GunBenchRecipe.Input> createRecipeValidator()
	{
		return new MenuRecipeValidatorRecipe<>(this.inventory.player.level())
		{
			...
			@Override
			public GunBenchRecipe.Input getInput(Container container, RecipeHolder<GunBenchRecipe> recipe)
			{
				return new GunBenchRecipe.Input(container);
			}

			@Override
			protected boolean test(RecipeHolder<GunBenchRecipe> holder, Container container, ServerPlayer player)
			{
	...
	@Override
	protected void setContainerByTransfer(HolderLookup.Provider provider, RecipeHolder<GunBenchRecipe> holder, CompoundTag payload)
	{
		super.setContainerByTransfer(provider, holder, payload);

		var ingredients = NBTUtils2.deserializeList(payload, "ingredients", ItemSerializationHelper.deserializerTag(provider));
		...
		this.inputContainer.setItem(BLUEPRINT_SLOT, ItemSerializationHelper.deserializeTag(provider, payload.getCompound("blueprint")));
	}

	@Override
	protected void onRecipeChanged(HolderLookup.Provider provider, GunBenchRecipe.Input input)
	{
