...
public class RepairMaterialTeachScreen extends TeachRecipeScreen<RepairMaterialTeachMenu, ItemStack>
{
	...
	{
		PacketDistributor.sendToServer(new RepairMaterialMaterialMessage(this.menu.getModulePos().getModuleView(), true, item));
	}
	...
	protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY)
	{
		...
		if (this.lastRecipe != recipe)
		{
			...
			if (recipe != null)
			{
				...
				this.materialText = Component.translatable("%s: %s", TEXT_MATERIAL_PREFIX, material.getDisplayName(PartTypes.MAIN.get()));
				this.tierText = Component.translatable("%s: %s", TEXT_TIER_PREFIX, TierHelper.getMaterialTier(material));
			}
