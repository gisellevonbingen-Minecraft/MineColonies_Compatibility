...
public class ButcherCategory extends JobBasedRecipeCategory<ButcherableIconCache>
{
	...
	@Override
	public void draw(ButcherableIconCache recipe, IRecipeSlotsView recipeSlotsView, PoseStack pose, double mouseX, double mouseY)
	{
		super.draw(recipe, recipeSlotsView, pose, mouseX, mouseY);

		this.arrow.draw(pose, ARROW_X, ARROW_Y);

		var tableIcons = recipe.getTableIcons();
		for (var i = 0; i < tableIcons.size(); i++)
		{
			RenderHelper.renderBlock(pose, tableIcons.get(i), BLOCK_X + i * 32, BLOCK_Y, 100, -30F, 30F, 16F);
		}
