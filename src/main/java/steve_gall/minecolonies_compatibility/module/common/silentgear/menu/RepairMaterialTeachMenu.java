...
import steve_gall.minecolonies_compatibility.module.common.silentgear.TierHelper;
import steve_gall.minecolonies_compatibility.module.common.silentgear.init.ModuleMenuTypes;
import steve_gall.minecolonies_tweaks.core.common.item.ItemSerializationHelper;

public class RepairMaterialTeachMenu extends TeachRecipeMenu<ItemStack, ItemStack>
{
	...
	public RepairMaterialTeachMenu(int windowId, Inventory inventory, RegistryFriendlyByteBuf buffer)
	{
	...
	@Override
	protected IMenuRecipeValidator<ItemStack, ItemStack> createRecipeValidator()
	{
		return new IMenuRecipeValidator<>()
		{
			...
			@Override
			public ItemStack getInput(Container container, ItemStack stack)
			{
				return stack;
			}

			@Override
			public ItemStack deserialize(HolderLookup.Provider provider, IFactoryController controller, CompoundTag tag)
			{
				return ItemSerializationHelper.deserializeTag(provider, tag);
			}

			@Override
			public CompoundTag serialize(HolderLookup.Provider provider, IFactoryController controller, ItemStack stack)
			{
				return ItemSerializationHelper.serializeTag(provider, stack);
			}
		};
	}
	...
	{
		var requiredLevel = TierHelper.getMaterialTier(stack);
	...
	@Override
	protected void onRecipeChanged(HolderLookup.Provider provider, ItemStack input)
	{
		if (this.recipe == null)
		{
			return;
