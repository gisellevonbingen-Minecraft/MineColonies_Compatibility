...
public class ApplePressFermentingTeachScreen extends TeachCraftingRecipeScreen<ApplePressFermentingTeachMenu, RecipeHolder<ApplePressFermentingRecipe>>
{
	...
	@Override
	protected ICustomizedRecipeStorage createRecipeStorage(RecipeHolder<ApplePressFermentingRecipe> recipe, List<ItemStorage> input)
	{
		var resultContainer = this.menu.getResultContainer();
		var container = new ItemStorage(ApplePressFermentingGenericRecipe.getContainer(recipe.value()));
		var output = resultContainer.getItem(0);
		return new ApplePressFermentingRecipeStorage(recipe.id(), input, container, output);
	}
