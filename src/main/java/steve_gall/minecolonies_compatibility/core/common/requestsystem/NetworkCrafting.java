...
public class NetworkCrafting implements IRequestableObject
{
	...
	public static NetworkCrafting deserialize(@NotNull IFactoryController controller, @NotNull CompoundTag tag)
	{
		var crafting = new NetworkCrafting();
		crafting.cycles = tag.getInt("cycles");
		crafting.item = ItemStack.of(tag.getCompound("item"));
		crafting.view = ItemStack.of(tag.getCompound("view"));
		crafting.text = Component.Serializer.fromJson(tag.getString("text"));
		return crafting;
	}

	public static void serialize(@NotNull IFactoryController controller, @NotNull CompoundTag tag, @NotNull NetworkCrafting request)
	{
		tag.putInt("cycles", request.cycles);
		tag.put("item", request.item.serializeNBT());
		tag.put("view", request.view.serializeNBT());
		tag.putString("text", Component.Serializer.toJson(request.text));
	}
